...
# 配置神经网络的参数。
LAYER1_NODE = 500           # 隐藏层节点数这个隐藏层有500个节点。
...
# inference辅助函数，给定神经网络的输入和所有参数，计算神经网络的前向传播结果。定义了一个使用ReLU激活函数的三层全连接神经网络。
# 通过加入隐藏层实现了多层网络结构，通过ReLU激活函数实现了去线性化。也支持传入用于计算参数平均值的类，这样方便在测试时使用滑动平均模型。
...
def train(mnist):
    ...
    # 计算在当前参数下神经网络前向传播的结果。这里给出的用于计算滑动平均的类为None所以的数不会使用参数的滑动平均值。
    y = inference(x, None, weights1, biases1, weights2, biases2)
    # 定义存储训练轮数的变量。这个变量不需要计算滑动平均值，所以这里指定这个变量为不可训练的变量
    # （trainable=Fasle）。在使用TensorFlow训练神经网络时，－般会将训练轮数的变量指定为不可训练的参数。
    ...
    # tf.trainablevariables返回的就是图上集合，GraphKeys.TRAINABLEVARIABLES中的元索。
    # 这个集合的元索就是所有没有指定trainable=False的参数。
    variables_averages_op = variable_averages.apply(tf.trainable_variables())
    # 计算使用了滑动平均之后的前向传播结果。第4章介绍过滑动平均不会改变变量本身的取值
    # 而是会维护一个影子变量来记录滑动平均值。所以当面要使用这个滑动平均值时，需要明确调用average函数。
    average_y = inference(x, variable_averages, weights1, biases1, weights2, biases2)
    # 计算交叉熵作为刻画预测值和真实值之间差距的损失函数。使用了TensorFlow中提供的sparse_softmax_cross_entropy_with_logits来计算交叉熵。
    # 当分类问题只打一个正确答案时，可以使用这个函数来加速交叉熵的计算。MNIST问题的图片中只n包含了
    # 0～9的－个数字，所以可以使用这个函数来计算交叉熵损失。这个函数的第一个参数是神经网络
    # 不包括softmax层的前向传播结果，第二个是训练数据的正确答案。因为标准答案是一个长度为10的一维数组，
    # 而该函数需要提供的是一个正确答案的数字，所以需要使用tf.argmax函数来得到正确答案对应的类别编号。
    ...
    learning_rate = tf.train.exponential_decay(LEARNING_RATE_BASE, global_step, mnist.train.num_examples / BATCH_SIZE,
                                               LEARNING_RATE_DECAY)
    # LEARNING_RATE_BASE基础的学习率，随着迭代的进行，更新变量时使用的学习率在这个基础上递减。
    # global_step当前途代的轮数。
    # mnist.train.num_examples / BATCH_SIZE过完所有的训练数据需要的迭代次数。
    # LEARNING_RATE_DECAY学习率衰减速度。
    # 使用tf.train.GradientDescentOptimizer优化算法来优化损失函数。注意这里损失函数

    # 包含了交叉熵损失和12正则化损失。
    train_step = tf.train.GradientDescentOptimizer(learning_rate).minimize(loss, global_step=global_step)
    # 在训练、神经网络模型时，每过一遍数据既需要通过反向传播来更新神经网络中的参数，
    # 又要更新每一个参数的滑到平均值。为了一次完成多个操作，TensorFlow提供了
